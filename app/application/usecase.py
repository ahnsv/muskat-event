...
class OrderUsecase(Application):
    def get_order(self, order_id: UUID, version_id: int = None) -> Order:
        try:
            found_order: Order = self.repository.get(order_id, version_id)
            return found_order
    ...
    def handle_payment(self, cmd: commands.HandlePayment) -> AggregateEvent:
        found_order = self.get_order(order_id=cmd.order_id)
